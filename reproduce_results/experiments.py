...
def add_order_of_magnitude(n, order):
   exp = np.log10(n)
   return 10**(exp + order) * n/10**(exp)
...
class Experiment:
    ...
    def run(self, start=0, input_atmosphere=None, method=1, rtol=1e-3,
            atol=1e-27, fast_and_loose=True, out2in=True, add_om_2_eddy=None):
        ...
        # add order of magnitude to eddy for sensitivity analysis
        if add_om_2_eddy:
            model.vars.edd = [add_order_of_magnitude(x, add_om_2_eddy) for x in model.vars.edd]
